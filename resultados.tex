...
El desarrollo produjo un framework funcional cuyos tres primeros incrementos
están consolidados y respaldados por pruebas, mientras que el cuarto se
encuentra parcialmente implementado.
...
completo con validadores y expresiones condicionales. El componente piloto
\texttt{/api-contract-pilot} demuestra el flujo completo (descubrimiento
$\rightarrow$ esquema $\rightarrow$ formulario $\rightarrow$ envío) y las
pruebas \texttt{test\_form\_schema.py} y \texttt{schema-to-formly.spec.ts}
cubren los casos relevantes.

\subsection{Incremento 4: bundle de esquemas (parcial)}
...
implementados y operativos. No obstante, este incremento se considera
\textbf{parcial}: el reflejo del polimorfismo de \texttt{FunctionalObject}
mediante \texttt{oneOf} y el de los \texttt{relationship()} del ORM como
\texttt{x-relationships} quedan pendientes, por lo que su cierre completo se
reformula como línea de trabajo futuro.
...
Generación automática de esquemas desde el ORM & Parcial \\ \hline
\end{tabular}
\end{table}
